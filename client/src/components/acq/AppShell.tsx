...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex">
      <aside className="hidden md:flex w-[240px] shrink-0 flex-col bg-sidebar text-sidebar-foreground p-4 gap-1 sticky top-0 h-screen border-r border-sidebar-border">
      ...
      <main className="flex-1 min-w-0">
        <div className="md:hidden sticky top-0 z-40 bg-sidebar text-sidebar-foreground border-b border-sidebar-border px-4 py-3 flex gap-3 overflow-x-auto">
          {NAV.map((n) => (
            <Link key={n.path} href={n.path}>
              <span className={cn("text-xs whitespace-nowrap", loc === n.path && "font-semibold text-sidebar-primary")}>{n.label}</span>
            </Link>
          ))}
        </div>
