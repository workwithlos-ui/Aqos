...
export function generateICMemo(a: DealAnalysis): ExportPayload {
  ...
  // Buyer-grade advisory sections
  lines.push(`## Buyer Cash Flow After Debt Service\n- After standby: ${a.buyerCashFlow.buyerCashFlow.display}\n- During standby: ${a.buyerCashFlow.buyerCashFlowDuringStandby.display}\n- Cash-on-cash return: ${a.buyerCashFlow.cashOnCashReturn.display}\n- Required CapEx reserve: ${money(a.buyerCashFlow.requiredCapEx)}\n- Working capital reserve: ${money(a.buyerCashFlow.workingCapitalReserve)}`);
  lines.push(`## Max Supportable Purchase Price\n- At 1.25x DSCR: ${money(a.maxSupportablePP.at1_25x)}\n- At 1.50x DSCR: ${money(a.maxSupportablePP.at1_50x)}\n- At 2.00x DSCR: ${money(a.maxSupportablePP.at2_00x)}\n- Current asking price supported: ${a.maxSupportablePP.priceIsSupported ? "Yes" : "No — price exceeds max supportable at 1.25x DSCR"}`);
  lines.push(`## Stress Test (${a.stressTest.stressRating})\n${a.stressTest.scenarios.map((s) => `- ${s.label}: DSCR during ${s.dscrDuringStandby.display} / after ${s.dscrAfterStandby.display} — ${s.pass ? "PASS" : `FAIL: ${s.failReason}`}`).join("\n")}`);
  lines.push(`## Refined Verdict\n**${a.refinedVerdict.verdict}**\n${a.refinedVerdict.buyerReason}${a.refinedVerdict.conditions.length ? "\n\nConditions:\n" + a.refinedVerdict.conditions.map((c) => `- ${c}`).join("\n") : ""}`);
  lines.push(`## Recommended Offer\n- Opening offer: ${money(a.recommendedOffer.openingOffer)}\n- Target price: ${money(a.recommendedOffer.targetPrice)}\n- Maximum price: ${money(a.recommendedOffer.maximumPrice)}\n- Preferred structure: ${a.recommendedOffer.preferredStructure}\n- Seller note amount: ${money(a.recommendedOffer.sellerNoteAmount)}\n- Earnout: ${a.recommendedOffer.earnoutAmount !== null ? money(a.recommendedOffer.earnoutAmount) : "none"}\n- Required transition period: ${a.recommendedOffer.requiredTransitionWeeks} weeks\n- Rationale: ${a.recommendedOffer.rationale}`);
  lines.push(`## Data Quality\n- Score: ${a.dataQuality.score}/100 (${a.dataQuality.label})\n- Critical gaps: ${a.dataQuality.criticalGaps.length ? a.dataQuality.criticalGaps.join("; ") : "none"}\n- Important gaps: ${a.dataQuality.importantGaps.length ? a.dataQuality.importantGaps.join("; ") : "none"}`);
